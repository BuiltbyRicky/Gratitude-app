...
async function scheduleLocalNotif(LocalNotifications, hour = 20, minute = 0) {
  ...
  const messages = [
    'Time to reflect on your day. 5 minutes is all it takes. ✨',
    'Your daily journal is waiting. How are you feeling right now? 🌿',
    'Take a moment for yourself. Today is worth remembering. 📖',
    'Good evening — a few minutes of reflection changes everything. 🕯️',
    'Your streak is counting on you. Let\u2019s journal together. 🔥',
  ];

  // Schedule 30 daily notifications (Capacitor limit workaround)
  const notifications = [];
  for (let i = 0; i < 30; i++) {
    const d = new Date();
...
// Goal-specific question pools — richer, more targeted
const GOAL_POOLS = {
  stress: [
...
function pickQs() {
  ...
  return [...FIXED_QS_L, shuffled[0], shuffled[1]];
}
